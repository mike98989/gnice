  ...
  function (
    ...
    $scope.get_staff_qualifications = function (serviceNo) {
      ...
      $.ajax({
        url:
          $scope.dirlocation +
          "api/get_staff_qualifications?serviceNo=" +
          serviceNo,
        type: "GET",
        async: true,
        cache: false,
    ...
    $scope.activate_or_deactivate = function (status, staff, index) {
      ...
      $.ajax({
        url:
          $scope.dirlocation +
          "api/activate_or_deactivate?status=" +
          status +
          "&serviceNo=" +
          staff.serviceNo +
          "&route=staff",
    ...
    $scope.verify_staff_record = function (staff) {
      ...
      $.ajax({
        url:
          $scope.dirlocation +
          "api/verify_staff_record?serviceNo=" +
          staff.serviceNo +
          "&route=staff",
        type: "POST",
        async: true,
      ...
      if (conf) {
        if ($scope.admin_data.account_type != "5") {
          ...
          $.ajax({
            url:
              $scope.dirlocation +
              "api/delete_staff_record?serviceNo=" +
              staff.serviceNo,
            async: true,
            cache: false,
            contentType: false,
      ...
      if (conf) {
        if ($scope.admin_data.account_type != "5") {
          ...
          $.ajax({
            url:
              $scope.dirlocation +
              "api/restore_staff_record?serviceNo=" +
              staff.serviceNo,
            async: true,
            cache: false,
            contentType: false,
            ...
            if (next < 4) {
              setTimeout(function () {
                ...
                window.location.href =
                  ...
                  "admindashboard/edit_profile?next=" +
                  next;
              }, 2000);
            } else {
              setTimeout(function () {
                //var next = ($scope.page*1)+1;
                window.location.href =
                  datagrab.completeUrlLocation + "admindashboard/staff";
              }, 2000);
            }
            $(".result").html(msg.message);
            $(".result").show();
    ...
    $scope.generate_id_card = function (staff_data) {
      ...
      qrcode.makeCode(
        "Name:" +
          ...
          staff_data.fname +
          " " +
          staff_data.othernames +
          " | ServiceNo:" +
          staff_data.serviceNo +
          " | Rank:" +
          staff_data.presentRank +
          " | BG:" +
          staff_data.blood_group +
          " | Img:" +
          staff_data.photo +
          " | Url: https://rims.corrections.gov.ng/staff?identity=" +
          staff_data.serviceNo
      );
